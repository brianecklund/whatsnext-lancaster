...
export default function HomeSplitClient({ events }: Props) {
  ...
  const selectedEventIndex = useMemo(() => {
    ...
    return orderedCalendarEvents.findIndex((e) => {
      if (selectedEvent.uid && e.uid) return e.uid === selectedEvent.uid;
      return e.id === selectedEvent.id;
    });
  }, [orderedCalendarEvents, selectedEvent]);

  const previousEvent =
    selectedEventIndex > 0 ? orderedCalendarEvents[selectedEventIndex - 1] : null;
  const nextEvent =
    selectedEventIndex >= 0 && selectedEventIndex < orderedCalendarEvents.length - 1
      ? orderedCalendarEvents[selectedEventIndex + 1]
      : null;
...
// stagger counter for left list
  ...
  function openRelativeEvent(direction: "prev" | "next") {
    if (selectedEventIndex < 0) return;
    const target = direction === "prev" ? previousEvent : nextEvent;
    if (!target) return;
    openSelected(target.uid ?? target.id);
  }
...
{leftDayGroups.length === 0 ? (
      ...
      >
        ...
        <div className="scroll mobileDetailBody">
          {selectedEvent ? (
        ...
        <div className="mobileDetailNav" aria-label="Browse events">
          <button
            className="mobileDetailNavBtn"
            type="button"
            onClick={() => openRelativeEvent("prev")}
          ...
          <button
            className="mobileDetailNavBtn"
            type="button"
            onClick={() => openRelativeEvent("next")}
            disabled={!nextEvent}
          >
            Next
          </button>
